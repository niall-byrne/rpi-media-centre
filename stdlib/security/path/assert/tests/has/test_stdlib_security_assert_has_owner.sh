...
@parametrize_with_arg_combos() {
  ...
  @parametrize \
    ...
    "extra_arg__________127,/etc|user1|extra_arg,127" \
    "null_path__________126,|user1,126" \
    "null_user__________126,/etc||,126" \
    "non_existent_path__126,non-existent|user1,126"
}

# shellcheck disable=SC2034
test_stdlib_security_path_assert_has_owner__@vary___________returns_expected_status_code() {
  local args=()
  _mock.create stdlib.logger.error
...
# shellcheck disable=SC2034
test_stdlib_security_path_assert_has_owner__@vary___________logs_expected_message() {
  local args=()
  _mock.create stdlib.logger.error
...
test_stdlib_security_path_assert_has_owner__valid_args_________non_matching__logs_an_error() {
  stdlib.security.path.assert.has_owner "/etc" "user1" 2> /dev/null

  stdlib.logger.error.mock.assert_called_once_with \
    "SECURITY: The ownership on '/etc' is not secure!"
  stdlib.logger.info.mock.assert_called_once_with \
    "Please consider running: sudo chown user1 /etc"
}
