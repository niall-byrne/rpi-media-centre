...
stdlib.array.assert.contains() {
  # $1: the array name
  # $2: the value to assert is present

  local return_code=0

  stdlib.array.query.contains "${@}" || return_code="$?"
  ...
  case "${return_code}" in
    ...
    126 | 127)
      ...
      ;;
    *)
      stdlib.logger.error "The value '${2}' is not found in the '${1}' array!"
      ;;
  esac
