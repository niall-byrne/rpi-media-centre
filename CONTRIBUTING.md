...
Under the hood the project uses [bashunit](https://github.com/TypedDevs/bashunit), so it's assertions will work in tests, along with the assertions built into the [mocking system](stdlib/testing/mock/mock.sh) and a group of included [custom assertions](stdlib/testing/assertion).
...
Use the referenced [custom capture functions](stdlib/testing/capture) to handle tasks such as:
...
Use the [mocking framework](stdlib/testing/mock/mock.sh) to create mocks in either the `setup`, or `setup_suite` test functions:
...
Be sure to refer to the [documentation](stdlib/testing/mock/mock.sh) on the mock object's behaviour and to see the complete list of available assertions.
...
stdlib.io.path.query.is.sh provides a suite of tools for this job.
