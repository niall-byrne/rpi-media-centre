...
_security_path_check_ownership() {
  ...
  if [[ "$(stat -c "%u" "${1}")" != "${RPI_SECURITY_REQUIRED_UID}" ]]; then
    _cli_log_error "SECURITY: The permissions on '${1}' are not secure!"
    _cli_log_info "Please consider running: sudo chown ${2}:${3} ${1}"
    return 127
  fi

  if [[ "$(stat -c "%g" "${1}")" != "${RPI_SECURITY_REQUIRED_GID}" ]]; then
    _cli_log_error "SECURITY: The permissions on '${1}' are not secure!"
    _cli_log_info "Please consider running: sudo chgrp ${3} ${1}"
    return 127
  fi
}
...
_security_path_check_permissions() {
  ...
  _io_ensure_vars_set "2" "${@}"

  if [[ "$(stat -c "%a" "${1}")" != "${2}" ]]; then
    _cli_log_error "SECURITY: The permissions on '${1}' are not secure!"
    _cli_log_info "Please consider running: sudo chmod ${2} ${1}"
    return 127
  fi
}
