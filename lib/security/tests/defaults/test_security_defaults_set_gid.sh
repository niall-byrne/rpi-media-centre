...
test_security_defaults_set_gid__@vary__calls_security_id_get_gid() {
  ...
  _security_id_get_gid.mock.assert_called_once_with "${EXISTING_GROUPNAME}"
}
...
test_security_defaults_set_gid__@vary__sets_gid_correctly() {
  ...
  RPI_SVC_GROUPNAME="${EXISTING_GROUPNAME}"
  RPI_SVC_GID="${EXISTING_GID}"
  _security_id_get_gid.mock.set.stdout "${EXPECTED_GID}"
