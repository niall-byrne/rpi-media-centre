...
_security_defaults_set_gid() {
  ...
  _security_warning_variable_mutated "RPI_SVC_GID" \
    "RPI_SVC_NEW_GID" \
    "RPI_SVC_GROUPNAME"
...
_security_defaults_set_groupname() {
  if [[ -n "${RPI_SVC_GROUPNAME}" ]]; then
    ...
    if ! getent group "${RPI_SVC_GROUPNAME}" > /dev/null 2>&1; then
      ...
      _cli_log_info "Please consider using the 'install account' command to provision it."
      return 127
    fi
  else
...
_security_defaults_set_uid() {
  ...
  _security_warning_variable_mutated "RPI_SVC_UID" \
    "RPI_SVC_NEW_UID" \
    "RPI_SVC_USERNAME"
