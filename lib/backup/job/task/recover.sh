...
_backup_job_task_recover_s3() {
  ...
  aws s3 cp "${RPI_BACKUP_JOB_REMOTE_TARGET}/${RPI_BACKUP_JOB_NAME}.tar" "${RPI_BACKUP_JOB_RECOVERY_PATH}/${RPI_BACKUP_JOB_NAME}-recovered.tar" "${RPI_BACKUP_JOB_UPLOAD_OPTIONS[@]}"
