...
_backup_job_task_tarball_filesystem_build() {
  ...
  sudo tar cf "${RPI_BACKUP_JOB_LOCAL_TARBALL_INCOMPLETE_FILENAME}" "$(basename "${RPI_BACKUP_JOB_LOCAL_SOURCE}")"
  sudo mv "${RPI_BACKUP_JOB_LOCAL_TARBALL_INCOMPLETE_FILENAME}" "${RPI_BACKUP_JOB_LOCAL_TARBALL_FINISHED_FILENAME}"
  sudo chmod "600" "${RPI_BACKUP_JOB_LOCAL_TARBALL_FINISHED_FILENAME}"
  sudo chown "${RPI_CONTAINER_UID}":"${RPI_CONTAINER_GID}" "${RPI_BACKUP_JOB_LOCAL_TARBALL_FINISHED_FILENAME}"
}
