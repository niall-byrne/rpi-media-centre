...
@parametrize_with_rsync_jobs() {
  @parametrize \
    "${1}" \
    "TEST_BACKUP_JOB_LOCAL_SOURCE;TEST_BACKUP_JOB_LOCAL_RSYNC_FOLDER;TEST_SVC_USERNAME;TEST_SVC_GROUPNAME;TEST_EXPECTED_TARGET_DIR" \
    "foo_bar__to__baz_qux________;/foo/bar;/baz/qux;user1;group1;/baz/qux/bar-rsync-backup" \
    "path1_path2__to__path3_path4;/path1/path2;/path3/path4;user2;group2;/path3/path4/path2-rsync-backup"
}
...
test_backup_job_task_rsync_filesystem__@vary__creates_backup_directory() {
  ...
  stdlib.security.path.make.dir.mock.assert_called_once_with \
    "1(${TEST_EXPECTED_TARGET_DIR}) 2(${TEST_SVC_USERNAME}) 3(${TEST_SVC_GROUPNAME}) 4(700)"
}
