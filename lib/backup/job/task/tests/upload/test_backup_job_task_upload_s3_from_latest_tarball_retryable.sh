...
@parametrize_with_inputs() {
  ...
  @parametrize \
    "${1}" \
    "TEST_LATEST_FILENAME;TEST_JOB_REMOTE_TARGET;TEST_JOB_NAME;TEST_UPLOAD_OPTIONS_DEFINITION;TEST_CONNECT_TIMEOUT;TEST_READ_TIMEOUT" \
...
test_backup_job_task_upload_s3_from_latest_tarball_retryable__@vary__calls_aws_cli_as_expected() {
  ...
  aws.mock.assert_called_once_with \
    "1(--cli-connect-timeout) 2(${TEST_CONNECT_TIMEOUT}) 3(--cli-read-timeout) 4(${TEST_READ_TIMEOUT}) 5(s3) 6(cp) 7(--no-progress) 8(${TEST_LATEST_FILENAME}) 9(${TEST_JOB_REMOTE_TARGET}/${TEST_JOB_NAME}.tar)$(stdlib.array.map.fn _fixture_upload_options_arg_string RPI_BACKUP_JOB_UPLOAD_OPTIONS)"
}
