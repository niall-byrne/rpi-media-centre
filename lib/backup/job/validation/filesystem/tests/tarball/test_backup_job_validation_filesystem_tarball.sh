...
test_backup_job_validation_filesystem_tarball__not_set_____________does_not_call_is_secure() {
...
# shellcheck disable=SC2034
test_backup_job_validation_filesystem_tarball__@vary__path_valid____calls_is_secure() {
  local RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER="${TEST_PATH}"
  local RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER_PERMISSION="${TEST_PERMISSION}"
...
# shellcheck disable=SC2034
test_backup_job_validation_filesystem_tarball__@vary__path_invalid__calls_is_secure() {
  local RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER="${TEST_PATH}"
  local RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER_PERMISSION="${TEST_PERMISSION}"
...
# shellcheck disable=SC2034
test_backup_job_validation_filesystem_tarball__@vary__path_valid____does_not_call_validation_error() {
  local RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER="${TEST_PATH}"
  local RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER_PERMISSION="${TEST_PERMISSION}"
...
# shellcheck disable=SC2034
test_backup_job_validation_filesystem_tarball__@vary__path_invalid__calls_validation_error() {
  local RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER="${TEST_PATH}"
  local RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER_PERMISSION="${TEST_PERMISSION}"
  ...
  _backup_job_validation_error.mock.assert_called_once_with \
    "1(Invalid tarball destination path.) 2(_backup_job_message_destination_path)"
}
