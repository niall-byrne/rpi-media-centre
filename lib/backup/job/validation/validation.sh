...
RPI_ALL_BACKUP_JOB_VALIDATORS_ARRAY=("argument" "dependency" "filesystem")
...
_backup_job_validation() {
  ...
  # $@: a set of validators to run (defaults to all)

  local backup_job_help_details_fn="${1}"
  local backup_job_log_valid_jobs_boolean="${2}"
  local backup_job_validator
  local backup_job_validators=()

  shift 2

  backup_job_validators=("$@")

  if [[ "${#backup_job_validators}" == "0" ]]; then
    backup_job_validators=("${RPI_ALL_BACKUP_JOB_VALIDATORS_ARRAY[@]}")
  fi
