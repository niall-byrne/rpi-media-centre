...
@parametrize_with_validator_sets() {
  ...
  @parametrize \
    "${1}" \
    "TEST_VALIDATOR_SET_DEFINITION;TEST_EXPECTED_VALIDATOR_SET_DEFINITION" \
    "default;;argument|dependency|filesystem" \
    "argument___no_dependency__no_filesystem;argument;argument" \
    "argument___dependency_____no_filesystem;argument|dependency;argument|dependency" \
    "argument___dependency_____filesystem___;argument|dependency|filesystem;argument|dependency|filesystem"
}

test_backup_job_validation__@vary__@vary__calls_backup_job_validation_argument_combinations() {
  local validator_args=()

  stdlib.array.make.from_string validator_args "|" "${TEST_VALIDATOR_SET_DEFINITION}"

  _backup_job_validation \
    _mock_help_fn \
    "${TEST_JOB_LOGGING_BOOLEAN_VALUE}" \
    "${validator_args[@]}" 2> /dev/null
