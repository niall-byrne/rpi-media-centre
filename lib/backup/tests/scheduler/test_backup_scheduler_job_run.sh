...
test_backup_scheduler_job_run__@vary__starts_the_job() {
  ...
  mock_job.mock.assert_called_once_with "1(${TEST_QUEUE1_NAME})"
}
...
test_backup_scheduler_job_run__@vary__logs_expected_error_messages() {
  ...
  _cli_log_error.mock.assert_calls_are \
    "1(BACKUP SCHEDULER: Backup job '${TEST_JOB_NAME}' - has failed the '${TEST_QUEUE1_NAME}' task !)" \
...
test_backup_scheduler_job_run__@vary__retries_the_job() {
  ...
  mock_job.mock.assert_call_n_is "1" "1(${TEST_QUEUE1_NAME})"
  mock_job.mock.assert_call_n_is "2" "1(${RPI_BACKUP_QUEUE_FAILED_TASK_EVENT})"
}
