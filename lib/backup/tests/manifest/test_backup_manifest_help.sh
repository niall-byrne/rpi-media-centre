...
test_backup_manifest_help__creates_columned_data() {
 ...
 RPI_BACKUP_JOB_LOCAL_SOURCE               |the local path to the backup source *(format this value as a 'path:permission' pair, i.e. /path:0755)
 RPI_BACKUP_JOB_LOCAL_RSYNC_FOLDER         |an optional local path to rsync the data to *(--delete is used) *(required when \`RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER\` is blank and \`RPI_BACKUP_JOB_REMOTE_TARGET\` is also blank) *(format this value as a 'path:permission' pair, i.e. /path:0755)
 RPI_BACKUP_JOB_LOCAL_TARBALL_FOLDER       |an optional local path to keep a tarball copy at *(required when \`RPI_BACKUP_JOB_LOCAL_RSYNC_FOLDER\` is blank and \`RPI_BACKUP_JOB_REMOTE_TARGET\` is also blank) *(required when \`RPI_BACKUP_JOB_LOCAL_TARBALL_VERSIONS\` is set) *(format this value as a 'path:permission' pair, i.e. /path:0755)
