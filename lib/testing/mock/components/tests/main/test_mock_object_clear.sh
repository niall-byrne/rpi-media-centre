...
test_mock_object__clear__clears_call_history() {
  test_mock.mock.clear

  assert_equals "0" "$(test_mock.mock.get.count)"
}

test_mock_object__clear__clears_side_effects() {
  test_mock.mock.clear
...
test_mock_object__clear__preserves_rc() {
  test_mock.mock.clear

  _capture_rc test_mock > /dev/null 2>&1

  assert_equals "1" "${TEST_RC}"
}

test_mock_object__clear__preserves_subcommand_with_stderr_and_stdout() {
  test_mock.mock.clear
...
test_mock_object__clear__preserves_pipeable() {
  test_mock.mock.set.pipeable "1"

  test_mock.mock.clear

  echo "piped arg" | test_mock > /dev/null 2>&1
  assert_equals "piped arg" "$(test_mock.mock.get.call "1")"
}
