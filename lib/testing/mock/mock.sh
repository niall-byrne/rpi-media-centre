...
_mock.compile() {
  ...
  MOCK_COMPONENT_FILE_SET=(
    "${RPI_WORKING_DIRECTORY}/lib/testing/mock/components/defaults.sh"
    "${RPI_WORKING_DIRECTORY}/lib/testing/mock/components/main.sh"
    "${RPI_WORKING_DIRECTORY}/lib/testing/mock/components/controller.sh"
    "${RPI_WORKING_DIRECTORY}/lib/testing/mock/components/getter.sh"
    "${RPI_WORKING_DIRECTORY}/lib/testing/mock/components/setter.sh"
    "${RPI_WORKING_DIRECTORY}/lib/testing/mock/components/assertion.sh"
  )

  # shellcheck disable=SC1090
  source <({
    echo "_mock.__generate_mock() {"
    echo "  __mock.persistence.create \"\${2}\" "
    echo "eval \"\$(cat <<EOF"
