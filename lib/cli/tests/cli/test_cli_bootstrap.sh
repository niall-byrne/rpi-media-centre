...
@parametrize_with_user_settings() {
  # $1: the test function to parametrize

  @parametrize \
    "${1}" \
    "TEST_CLI_COMPILATION_PATH;TEST_USERNAME;TEST_GROUPNAME" \
    "scenario1;/tmp/cli1.sh;test_user1;test_group1" \
    "scenario2;/tmp/cli2.sh;test_user2;test_group2"
}
...
test_cli_bootstrap__@vary__compilation_required______not_memory_only__calls_path_secure() {
  ...
  stdlib.security.path.secure.mock.assert_called_once_with \
    "1(${TEST_CLI_COMPILATION_PATH}) 2(${TEST_USERNAME}) 3(${TEST_GROUPNAME}) 4(640)"
}
