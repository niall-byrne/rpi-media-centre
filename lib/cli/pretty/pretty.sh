...
_cli_pretty_brackets_style_1() {
  ...
  __cli_pretty_brackets "${THEME_BRACKETS_STYLE_1}" \
...
_cli_pretty_brackets_style_2() {
  ...
  __cli_pretty_brackets "${THEME_BRACKETS_STYLE_2}" \
...
_cli_pretty_columns() {
  ...
  local _STDLIB_WRAP_PREFIX_STRING="  "
  ...
  while IFS= read -r CONTENT_LINE; do
    ...
    stdlib.string.justify.left_var "${RPI_CLI_COLUMN_ALIGNMENT_WITHOUT_TAB}" "RPI_CLI_PRETTY_COLUMN_1"
    _cli_pretty_colour_n "ENTITY" "${RPI_CLI_PRETTY_COLUMN_1}"
    stdlib.string.wrap "${RPI_CLI_COLUMN_ALIGNMENT_WITHOUT_TAB}" 100 "- ${RPI_CLI_PRETTY_COLUMN_2}" |
