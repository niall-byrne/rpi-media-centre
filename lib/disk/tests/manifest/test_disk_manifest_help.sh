...
test_disk_manifest_help__creates_columned_data() {
  ...
  _cli_pretty_columns_pipe.mock.assert_called_once_with \
    "1( RPI_DISK_UUID        |the \`UUID\` of the disk (find with: sudo blkid)
 RPI_DISK_NAME        |a unique name for this disk
 RPI_DISK_CRYPT_GROUP |an optional identifier for disks that share a luks password
 RPI_DISK_MOUNT_POINT |a valid mount point for this disk on the filesystem)"
}
