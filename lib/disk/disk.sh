...
_disk_initialize_mounts() {
  ...
  if _service_query_is_selected "plex"; then
...
_disk_unlock() {
  ...
  if ! _is_disk_mounted "${RPI_DISK_UUID}" "${RPI_DISK_NAME}" "${RPI_DISK_MOUNT_POINT}"; then
    stdlib.security.path.make.dir "${RPI_DISK_MOUNT_POINT}" "${RPI_SVC_USERNAME}" "${RPI_SVC_GROUPNAME}" "700"
...
_disk_unlock_with_crypt_group() {
  ...
  if [[ "${RPI_DISK_CRYPT_PASSWORD}" == $'\\0' ]]; then
    ...
    _STDLIB_PASSWORD_BOOLEAN=1 \
      stdlib.io.stdin.prompt RPI_DISK_CRYPT_PASSWORD "Enter the password for disk group '${RPI_DISK_CRYPT_GROUP}': "
